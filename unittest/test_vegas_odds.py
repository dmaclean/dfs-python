...
class TestVegasOdds(unittest.TestCase):
	...
	def test_process(self):
		...
		result = self.testUtil.select_from_vegas({ "date": "2014-01-01", "road_team": "DAL", "home_team": "WAS" })
		self.assertTrue(result[0] == 2.0 and result[1] == -2.0 and result[2] == 204.0 and result[3] == 101.0 and result[4] == 103.0)
		
		# Indiana at Toronto
		result = self.testUtil.select_from_vegas({ "date": "2014-01-01", "road_team": "IND", "home_team": "TOR" })
		self.assertTrue(result[0] == -5.0 and result[1] == 5.0 and result[2] == 191.5 and result[3] == 98.25 and result[4] == 93.25)
		
		# New Orleans at Minnesota
		result = self.testUtil.select_from_vegas({ "date": "2014-01-01", "road_team": "NOP", "home_team": "MIN" })
		self.assertTrue(result[0] == 5.5 and result[1] == -5.5 and result[2] == 211.5 and result[3] == 103 and result[4] == 108.5)
		
		# Philly at Denver
		result = self.testUtil.select_from_vegas({ "date": "2014-01-01", "road_team": "PHI", "home_team": "DEN" })
		...
		result = self.testUtil.select_from_vegas({ "date": "2014-01-01", "road_team": "CHA", "home_team": "LAC" })
